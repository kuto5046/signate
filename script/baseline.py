...
def feature_arrange(df, type):
    ...
    passed = pd.DataFrame(passed[0], columns=["Passed"]).astype(int)
    ...
    if type == "train":
        target = df['target']
        new_df = pd.concat([target, place, passed, angle, area, max_floor, material,
                            bath, kitchen, facility, internet], axis=1)
    else:
        new_df = pd.concat([place, passed, angle, area, max_floor, material,
                            bath, kitchen, facility, internet], axis=1)
    return new_df
...
cv_score = 0
X_train, X_valid, y_train, y_valid = train_test_split(X, y, shuffle=True, random_state=0)
